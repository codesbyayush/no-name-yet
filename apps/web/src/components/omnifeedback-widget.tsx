...
export function OmniFeedbackWidget({
    ...
    const initializeWidget = () => {
      try {
        if (typeof window.OmniFeedbackWidget === 'undefined') {
          console.warn('OmniFeedbackWidget not loaded yet. Make sure the script is included.');
          return;
        }
        ...
        const options = {
          ...
          jwtAuthToken,
          theme,
          targetElement: targetElement || (containerRef.current ? containerRef.current : 'body'),
        };
        ...
        if (onFeedbackSubmitted) {
          ...
          window.addEventListener('omnifeedback:submitted' as any, handleFeedbackSubmitted);
          
          // Cleanup function will remove this listener
          return () => {
            window.removeEventListener('omnifeedback:submitted' as any, handleFeedbackSubmitted);
          };
        }
      } catch (error) {
        console.error('Failed to initialize OmniFeedback widget:', error);
        onError?.(error instanceof Error ? error.message : 'Failed to initialize widget');
      }
    };
  ...
  }, [tenantId, apiUrl, jwtAuthToken, theme, targetElement, onFeedbackSubmitted, onError]);
