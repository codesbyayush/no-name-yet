...
import { useOnboardingStatus, useSubdomainContext, shouldRedirectToOnboarding } from "@/hooks/use-onboarding-new";
...
export function OnboardingGuard({ children, requiresOnboarding = false }: OnboardingGuardProps) {
  ...
  useEffect(() => {
    ...
    const redirectDecision = shouldRedirectToOnboarding(onboardingStatus, subdomainContext);
  ...
  }, [session, onboardingStatus, subdomainContext, location.pathname, navigate, requiresOnboarding, sessionLoading, onboardingLoading]);
